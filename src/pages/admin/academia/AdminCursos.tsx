...
export default function AdminCursos() {
  ...
  async function addResource(event: React.FormEvent) {
    ...
    if (!lesson || !resourceForm.title.trim()) return;
    if (!resourceFile && !resourceForm.file_url.trim()) return toast.error('Sube un archivo o ingresa una URL.');

    setUploadingResource(true);
    try {
      let finalUrl = resourceForm.file_url;
      if (resourceFile) {
        const uploaded = await academyAPI.uploadAdminResource(lessonDraft.moduleId, lesson.id, resourceFile);
        finalUrl = uploaded.publicUrl;
      }
      
      const resource = await academyAPI.createAdminResource({ ...resourceForm, file_url: finalUrl, file_name: resourceFile ? resourceFile.name : resourceForm.file_name, lesson_id: lesson.id, description: '', sort_order: resources.length + 1 });
